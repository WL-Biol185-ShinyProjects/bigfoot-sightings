...
create_moon_plot <- function(phase, sighting_count) {
  ...
  p <- ggplot() +
    ...
    geom_polygon(data = df_circle, aes(x = x, y = y), 
                 fill = "#2c3e50", colour = "#7f8c8d", linewidth = 2) +
    # Illuminated side
    geom_polygon(data = df_illum, aes(x = x, y = y), 
                 fill = "#f9ca24", colour = "#7f8c8d", linewidth = 1) +
    # Add sighting count
    geom_text(aes(x = 0, y = -1.5, 
                  label = paste("Sightings:", sighting_count)), 
              colour = "#f9ca24", size = 8, fontface = "bold") +
    coord_fixed() +
    theme_void() +
    theme(
      plot.background = element_rect(fill = "#0a0e27", colour = NA),
      panel.background = element_rect(fill = "#0a0e27", colour = NA)
    ) +
    xlim(-1.2, 1.2) +
    ylim(-1.7, 1.2)
